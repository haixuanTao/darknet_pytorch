...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        anchor_w = (
            anchor_w.repeat(nB, 1).repeat(1, 1, nH * nW).view(nB * nA * nH * nW)
        )
        anchor_h = (
            anchor_h.repeat(nB, 1).repeat(1, 1, nH * nW).view(nB * nA * nH * nW)
        )
        ...
        loss_conf = (
            nn.MSELoss(reduction="sum")(conf * conf_mask, tconf * conf_mask)
            / 2.0
        )
